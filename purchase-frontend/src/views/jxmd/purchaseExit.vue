  ...
  <div class="entity-page">
    <section class="entity-card entity-card--form">
      <div class="section-head">
        <h3>{{ dataDialogForm.id ? "编辑采购退货" : "新增采购退货" }}</h3>
      ...
      <el-form ref="purchaseExitForm" :model="dataDialogForm" :rules="rules" label-position="top" class="entity-form">
        <div class="form-grid">
          <el-form-item label="采购单号" prop="number">
            <el-select v-model="dataDialogForm.number" filterable placeholder="请选择采购单">
              <el-option
                v-for="item in purchaseOptions"
                :key="item.value"
                :label="item.value + ' / ' + item.address"
                :value="item.value"
              ></el-option>
            </el-select>
          </el-form-item>
        ...
        <el-button type="primary" class="save-btn" @click="handleSubmitFormData('purchaseExitForm')">
    ...
    <section class="entity-card entity-card--list">
      <div class="section-head">
        ...
        <div class="list-actions">
          <el-input v-model="dataForm.select" clearable placeholder="搜索采购单号" @keyup.enter.native="getDataList"></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        number: [{ required: true, message: "请选择采购单号", trigger: "change" }],
  ...
  methods: {
    ...
    handleSubmitFormData(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.$http
          .post("/purchaseExit/add", payload)
          .then(() => {
            this.$message.success(this.dataDialogForm.id ? "退货单已更新" : "退货单已创建");
            this.resetForm();
            this.getDataList();
          })
          .finally(() => {
    ...
    handleDelete(index, item) {
      this.$confirm("确认删除当前采购退货单吗？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => this.$http.post(`/purchaseExit/delete/?id=${item.id}`))
        .then(() => {
          this.$message.success("采购退货单已删除");
          this.resetForm();
          this.getDataList();
        })
        .catch(() => {});
    ...
    resetForm() {
      ...
      if (this.$refs.purchaseExitForm) {
        this.$refs.purchaseExitForm.resetFields();
      }
    },
    loadAll() {
