  ...
  <div class="entity-page">
    <section class="entity-card entity-card--form">
      <div class="section-head">
        <h3>{{ dataDialogForm.id ? "编辑入库单" : "新增入库单" }}</h3>
        <div class="head-actions">
          <button type="button" class="plain-btn" @click="startCreate">新增入库</button>
          <button type="button" class="plain-btn" @click="resetForm">清空表单</button>
        </div>
      </div>

      <el-form ref="depositoryForm" :model="dataDialogForm" label-position="top" class="entity-form">
        <div class="form-grid">
          ...
          <el-form-item label="入库数量">
            <el-input v-model="dataDialogForm.shopNumber" placeholder="请输入入库数量"></el-input>
          </el-form-item>
          ...
          <el-form-item label="入库单价">
            <el-input v-model="dataDialogForm.shopPrice" placeholder="请输入入库单价"></el-input>
          </el-form-item>
        </div>
      ...
      <div class="entity-table">
        ...
        <div v-for="item in dataList" :key="item.id" class="entity-table__row">
          ...
          <span class="status-pill" :class="{ 'is-done': Number(item.status) === 0 }">
            {{ Number(item.status) === 0 ? "已入库" : "待质检" }}
          </span>
          <span class="row-actions">
            <button type="button" class="mini-btn" @click="handleEdit(0, item)">编辑</button>
            <button type="button" class="mini-btn" :disabled="Number(item.status) === 0" @click="check(0, item)">质检</button>
  ...
  methods: {
    ...
    handleEdit(index, item) {
      this.dataDialogForm = {
        ...
        status: item.status,
        inUser: sessionStorage.getItem("username") || "",
        remark: item.remark,
      };
    },
    handleSubmitFormData() {
      ...
      this.$http
        .post("/depositoryIn/add", payload)
        .then(() => {
          this.$message.success(this.dataDialogForm.id ? "入库单已更新" : "入库单已创建");
          this.resetForm();
          this.getDataList();
        })
        .finally(() => {
    ...
    handleDelete(index, item) {
      this.$confirm("确认删除当前入库记录吗？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => this.$http.post(`/depositoryIn/delete/?id=${item.id}`))
        .then(() => {
          this.$message.success("入库记录已删除");
          this.resetForm();
          this.getDataList();
        })
        .catch(() => {});
    ...
    check(index, item) {
      this.$confirm("确认质检后将正式存入仓库，是否继续？", "提示", {
        ...
        .then(() => this.$http.post(`/depositoryIn/checkById?id=${item.id}`))
        .then(() => {
          this.$message.success("商品质检完成");
          this.getDataList();
        })
        .catch(() => {});
    },
